...
Multi-Query Attention 的核心改动很小:保留多组 Query head,但让多组 Query 共享更少组 Key/Value head。标准 Multi-Head Attention 中,Q、K、V 的 head 数通常相同;MQA 中,K/V head 数可以压到 1。实际工程里也常见折中版本,即 K/V head 数大于 1 但小于 Q head 数,通常称为 Grouped-Query Attention。

当前代码里的 `MultiQueryAttention(hidden_dim = 128, nums_head = 8, nums_key_value_head = 4)` 严格说更接近 GQA:8 个 Query head 被分成 4 组,每组共享同一个 K/V head。若把 `nums_key_value_head` 改成 1,才是经典 MQA。
...
区别在于 K/V 的来源少了。推理阶段做 KV Cache 时,缓存 K/V 的 head 数从 `N_q` 降为 `N_{kv}`。这就是 MQA/GQA 对长序列推理更有价值的地方:少存 K/V,少读 K/V,显存带宽压力更小。
...
更重要的是 KV Cache。自回归推理时,每生成一个 token 都要缓存历史 K/V。标准 MHA 的 K/V cache 规模与 `N_q` 成正比,MQA/GQA 与 `N_{kv}` 成正比:

$$
\text{KVCache} \propto B \times S \times N_{kv} \times D
$$

这也是面试中更值得强调的收益:MQA/GQA 主要优化推理阶段的 KV Cache 存储和读取,不是让 attention 分数矩阵从 `(B, N_q, S, S)` 变小。attention weights 仍然按 Query head 计算。
...
> MQA 保留多个 Query head,但减少 Key/Value head 的数量。标准 MHA 中每个 Query head 都有独立的 K/V head;MQA 中多个 Query head 共享同一组 K/V。这样 attention weights 的 head 数仍然等于 Query head 数,但 K/V 投影参数量和推理时的 KV Cache 会下降。当前代码里 `nums_head = 8`,`nums_key_value_head = 4`,所以它更像 GQA:每 2 个 Query head 共享 1 个 K/V head。实现上先把 Q reshape 成 `(B, 8, S, D)`,K/V reshape 成 `(B, 4, S, D)`,再通过 `repeat_interleave` 沿 head 维复制成 `(B, 8, S, D)`,最后按标准 scaled dot-product attention 计算。

如果被追问为什么能加速推理,不要只说"计算量变少"。更准确的说法是:MQA/GQA 显著减少 KV Cache 的存储和读取压力,尤其在长上下文自回归解码时收益明显。attention 矩阵本身仍是 `(B, nums_head, seq_length, seq_length)`,并没有因为 K/V head 少了而直接变成 `(B, nums_key_value_head, seq_length, seq_length)`。
